    ...
    public boolean isNStraightHand(int [] hand, int W){
        ...
        for (int card: hand){
            if (!card_counts.containsKey(card)){
                ...
                card_counts.replace(card, card_counts.get(card) + 1); // count nya nambah
            }
        }
        while(card_counts.size() > 0){
            int min_val = card_counts.firstKey();
            for (int card = min_val; card < min_val + W; card++){
                if(!card_counts.containsKey(card)) return false;  // if the consecutive is not thaere, we return false
